...
!!! info
    The style guide is always a work in progress, and not all EasyJobRecipes code
    follows the rules. When modifying EasyJobRecipes, please fix the style violations
...
EasyJobRecipes uses [JuliaFormatter](https://github.com/domluna/JuliaFormatter.jl) as
an auto-formatting tool.

We use the options contained in [`.JuliaFormatter.toml`](https://github.com/MineralsCloud/EasyJobRecipes.jl/blob/main/.JuliaFormatter.toml).

To format your code, `cd` to the EasyJobRecipes directory, then run:
...
!!! info
    A continuous integration check verifies that all PRs made to EasyJobRecipes have

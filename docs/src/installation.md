...
[EasyJobRecipes](https://github.com/MineralsCloud/EasyJobRecipes.jl).
...
2. Run the following commands and wait for them to finish:
   ...
   julia> Pkg.add("EasyJobRecipes")
   ```

3. Run

   ```julia-repl
   julia> using EasyJobRecipes
   ```
...
version of EasyJobRecipes, type

```@repl
using Pkg
Pkg.update()
pkg"add https://github.com/MineralsCloud/EasyJobRecipes.jl"
```

in the second step above.

## Update EasyJobRecipes
...
our [GitHub repository](https://github.com/MineralsCloud/EasyJobRecipes.jl)
for new releases.
Once we release a new version, you can update EasyJobRecipes by typing

```@repl
using Pkg
Pkg.update("EasyJobRecipes")
Pkg.gc()
```

in the Julia REPL.

## Uninstall and reinstall EasyJobRecipes
...
1. To uninstall, in a Julia session, run
   ...
   julia> Pkg.rm("EasyJobRecipes")

   julia> Pkg.gc()
   ```

2. Press `ctrl+d` to quit the current session. Start a new Julia session and
   reinstall EasyJobRecipes.
